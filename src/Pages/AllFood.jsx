...
const AllFood = () => {
  ...
  useEffect(() => {
    setErrorMsg(null);
    let url = `https://seasons-server.vercel.app/api/v1/all-foods?page=${currentPage}&size=${itemPerPage}&t=${Date.now()}`;
    if (search) {
      url = `https://seasons-server.vercel.app/api/v1/all-foods?fname=${search}&page=${currentPage}&size=${itemPerPage}&t=${Date.now()}`;
    }
    axios.get(url)
      .then(res => {
  ...
  }, [currentPage, search, itemPerPage])
  ...
  return (
    <>
      ...
      <Container>
        {errorMsg && <div className="text-center text-red-500 font-bold text-2xl my-10">Error Loading Foods: {errorMsg}</div>}
        {(!allfoods || allfoods.length === 0) && !errorMsg && <div className="text-center text-gray-500 font-bold text-2xl my-10">No foods found.</div>}
