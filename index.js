...
// :::::::: ADMIN APIS (With Dummy Data) ::::::::::::::::
    app.get('/api/v1/admin/stats', async (req, res) => {
      try {
        const usersCount = await userCollection.countDocuments();
        const foodsCount = await allFoodCollection.countDocuments();
        const ordersCount = await orderCollection.countDocuments();

        // Dummy data for admin panel visualization
        const dummyRevenue = 45000;
        const dummyChartData = [
          { month: 'Jan', revenue: 4000, orders: 120 },
          { month: 'Feb', revenue: 3000, orders: 98 },
          { month: 'Mar', revenue: 8000, orders: 200 },
          { month: 'Apr', revenue: 2780, orders: 105 },
          { month: 'May', revenue: 1890, orders: 75 },
          { month: 'Jun', revenue: 2390, orders: 89 },
        ...
        res.send({
          ...
          totalFoods: foodsCount || 45,
          totalOrders: ordersCount || 320,
        ...
        console.error(error);
        res.status(500).send({ message: "Internal server error", error: error.message, stack: error.stack });
      }
    });
    ...
    app.get('/itemCount', async (req, res) => {
      const count = await allFoodCollection.countDocuments()
      res.send({ count })
    })
